...
async def telegram_get_me() -> dict:
    """Verify TELEGRAM_BOT_TOKEN — returns bot identity (username, id, name)."""
    with tool_call("telegram_get_me", provider="telegram") as ctx:
        try:
            return {"data": await _tg.get_me()}
        except FinanceError as e:
            ctx["error"](e.code.value)
            return e.to_dict()
...
async def telegram_send_message(chat_id: str, text: str,
    ...
    """Send text to a Telegram chat. parse_mode: 'Markdown' | 'HTML' | None."""
    with tool_call("telegram_send_message", provider="telegram") as ctx:
        try:
            return {"data": await _tg.send_message(chat_id, text, parse_mode)}
        except FinanceError as e:
            ctx["error"](e.code.value)
            return e.to_dict()
...
@mcp.tool()
async def telegram_send_alert(text: str, parse_mode: str | None = None) -> dict:
    """Send to TELEGRAM_ALERT_CHAT_ID (env). Use for notifications without knowing chat_id."""
    with tool_call("telegram_send_alert", provider="telegram") as ctx:
        chat_id = os.getenv("TELEGRAM_ALERT_CHAT_ID", "").strip()
        if not chat_id:
            e = FinanceError(ErrorCode.AUTHENTICATION_FAILED,
                             "TELEGRAM_ALERT_CHAT_ID not set", provider="telegram")
            ctx["error"](e.code.value)
            return e.to_dict()
        try:
            return {"data": await _tg.send_message(chat_id, text, parse_mode)}
...
@mcp.tool()
async def telegram_get_updates(offset: int | None = None, limit: int = 100,
                               timeout: int = 0) -> dict:
    """Poll incoming updates. offset = last update_id + 1 (ack prior batch).
    timeout > 0 = long-poll (server holds request up to N seconds)."""
    with tool_call("telegram_get_updates", provider="telegram") as ctx:
        try:
            allowed = os.getenv("TELEGRAM_ALLOWED_USERS", "").strip()
            allow_set = {u.strip() for u in allowed.split(",") if u.strip()}
            updates = await _tg.get_updates(offset, limit, timeout)
            if allow_set:
                updates = [
                    u for u in updates
                    if str((u.get("message", {}) or {}).get("from", {}).get("id"))
                    in allow_set
                ]
            return {"data": {"updates": updates, "count": len(updates)}}
        except FinanceError as e:
            ctx["error"](e.code.value)
            return e.to_dict()
